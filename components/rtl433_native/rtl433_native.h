...
#endif
{
 ...
 protected:
  ...
  std::string pending_last_packet_value_{};
  ...
  std::optional<uint32_t> scheduled_stale_publish_at_ms_{};
  std::size_t restore_saved_state_index_{0};
  ...
  void queue_restore_saved_states();
  void restore_next_saved_state();
  bool restore_saved_state(const std::string &logical_key);
  void sync_time_base();
  ...
  void queue_diagnostic_publish(const std::string &last_packet_value);
