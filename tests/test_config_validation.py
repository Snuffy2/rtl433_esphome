...
def cpp_function_body(source: str, signature: str) -> str:
    """Return the body text for a top-level C++ function in source."""
...
def test_gateway_state_publish_uses_stale_cache_for_restored_values() -> None:
    ...
    source = Path("components/rtl433_native/rtl433_native.cpp").read_text()
    publish_state = cpp_function_body(source, "void Gateway::publish_state(")
    publish_stale_states = cpp_function_body(source, "void Gateway::publish_stale_states(")

    assert "void Gateway::publish_stale_state(" in source
    assert "this->publish_stale_state(logical_key, entities, millis());" in publish_state
    assert "this->publish_stale_state(logical_key, entities, now);" in publish_stale_states
...
def test_gateway_reprojects_pending_restored_state_after_time_sync() -> None:
    ...
    restore_saved_states = cpp_function_body(source, "void Gateway::restore_saved_states(")
    sync_time_base = cpp_function_body(source, "void Gateway::sync_time_base(")
    reproject_pending_restored_states = cpp_function_body(
        source, "void Gateway::reproject_pending_restored_states("
    )
    ...
    assert (
        ...
        in reproject_pending_restored_states
    )
    assert (
        "item = this->pending_clock_age_restore_.erase(item);" in reproject_pending_restored_states
    )
